...
from ..models.wan_video_dit import RMSNorm, sinusoidal_embedding_1d, WanModel
...
class WanUniAnimateLongVideoPipeline(BasePipeline):
    ...
    def __call__(
        ...
        context_step = min(1, int(np.ceil(np.log2(noise.shape[2] / context_size))) + 1)
        num_frames = noise.shape[2]
        context_queue[-1] = [
            e % num_frames
            for e in range(
                num_frames - context_size * context_step, num_frames, context_step
            )
        ]
        import math
        ...
        if input_image is not None and self.image_encoder is not None:
            ...
            image_emb = self.encode_image(
                input_image, context_size * 4 - 3, height, width
            )
        else:
            image_emb = {}
        ...
        for ii in global_context:
            ...
            dwpose_data_per = dwpose_data[
                :, :, (ii[0][0] * 4) : (ii[0][-1] * 4 + 1), :, :
            ]
            dwpose_data_list.append(
                self.dwpose_embedding(
                    (
                        torch.cat(
                            [
                                dwpose_data_per[:, :, :1].repeat(1, 1, 3, 1, 1),
                                dwpose_data_per,
                            ],
                            dim=2,
                        )
            ...
            #
            first_feature_per_seg.append(
                torch.randn_like(latents[:, :, ii[0][0] : (ii[0][0] + 2)])
            )
        ...
        for progress_id, timestep in enumerate(
            ...
            for i_index, context in enumerate(global_context):
                ...
                condition = rearrange(
                    dwpose_data_list[i_index], "b c f h w -> b (f h w) c"
                ).contiguous()
                ...
                for j, c in enumerate(context):
                    ...
                    if i_index == 0 and j == 0:
                        ...
                        noise_pred_out[:, :, c] = (
                            noise_pred_out[:, :, c] + noise_pred[j : j + 1]
                        )
                    else:
                        ...
                        counter[:, :, c] = counter[:, :, c] + 1
                        noise_pred_out[:, :, c] = (
                            noise_pred_out[:, :, c] + noise_pred[j : j + 1, :, 2:]
                        )
...
class WanRepalceAnyoneVideoPipeline(BasePipeline):
    ...
    def fetch_models(self, model_manager: ModelManager):
        ...
        self.learn_in_embedding = nn.Sequential(
            nn.Conv3d(
                4, concat_dim * 4, (3, 3, 3), stride=(1, 1, 1), padding=(1, 1, 1)
            ),
            nn.SiLU(),
            nn.Conv3d(
                concat_dim * 4,
        ...
        self.inpaint_embedding = nn.Sequential(
            nn.Conv3d(
                16, concat_dim * 4, (3, 3, 3), stride=(1, 1, 1), padding=(1, 1, 1)
            ),
            nn.SiLU(),
        ...
        for key in model_manager.state_dict_new_module:
            if "inpaint_embedding" in key:
                state_dict_new[key.split("inpaint_embedding.")[1]] = (
                    model_manager.state_dict_new_module[key]
                )
        #
        ...
        for key in model_manager.state_dict_new_module:
            if "learn_in_embedding" in key:
                state_dict_new[key.split("learn_in_embedding.")[1]] = (
                    model_manager.state_dict_new_module[key]
                )
    ...
    def __call__(
        ...
        segmentation_data = (batch["segmentation_data"] / 255.0 > 0).unsqueeze(
            0
        )  # [1, 81, 832, 480]
        with torch.no_grad():
            #
            # latents = self.encode_video(input_video, **tiler_kwargs).to(dtype=self.torch_dtype, device=self.device)
            latents_masked_encode = self.encode_video(
                (video * (~segmentation_data)).to(
                    dtype=self.torch_dtype, device=self.device
                ),
                **tiler_kwargs,
            ).to(dtype=self.torch_dtype, device=self.device)
        masked_video = torch.cat(
            [(video * (~segmentation_data)), (~segmentation_data).float()], dim=1
        )
        self.learn_in_embedding.to(torch.bfloat16).to(self.device)
        self.inpaint_embedding.to(torch.bfloat16).to(self.device)
        masked_video = self.learn_in_embedding(
            (
                torch.cat(
                    [masked_video[:, :, :1].repeat(1, 1, 3, 1, 1), masked_video], dim=2
                )
            )
            .to(torch.bfloat16)
            .to(self.device)
        )
        latents_masked = self.inpaint_embedding(
            latents_masked_encode.to(self.device)
        )  # .unsqueeze(0)
        ...
        for progress_id, timestep in enumerate(
            ...
            noise_pred_posi = model_fn_wan_video(
                self.dit,
                latents + masked_video + latents_masked,
                timestep=timestep,
                **prompt_emb_posi,
                **image_emb,
                **extra_input,
