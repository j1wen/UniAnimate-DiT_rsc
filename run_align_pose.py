    ...
    def __call__(self, oriImg):
        ...
        with torch.no_grad():
            ...
            if -1 not in bodyfoot_score[:, 18] and -1 not in bodyfoot_score[:, 19]:
                bodyfoot_score[:, 18] = np.array([18.0])
            else:
                bodyfoot_score[:, 18] = np.array([-1.0])
            if -1 not in bodyfoot_score[:, 21] and -1 not in bodyfoot_score[:, 22]:
                bodyfoot_score[:, 19] = np.array([19.0])
            else:
                bodyfoot_score[:, 19] = np.array([-1.0])
...
def dw_func(
    ...
    frame,
    dwpose_model,
    dwpose_woface_folder="tmp_dwpose_wo_face",
    dwpose_withface_folder="tmp_dwpose_with_face",
):
...
def mp_main(args):
    ...
    if args.source_video_paths.endswith("mp4"):
        ...
        # video list
        video_paths = [
            os.path.join(args.source_video_paths, frame_name)
            for frame_name in os.listdir(args.source_video_paths)
        ]
    ...
    ########neck########
    l_neck_ref = (
        (ref_candidate[0][0] - ref_candidate[1][0]) ** 2
        + (ref_candidate[0][1] - ref_candidate[1][1]) ** 2
    ) ** 0.5
    l_neck_0 = (
        (candidate[0][0] - candidate[1][0]) ** 2
        + (candidate[0][1] - candidate[1][1]) ** 2
    ) ** 0.5
    neck_ratio = l_neck_ref / l_neck_0

    x_offset_neck = (candidate[1][0] - candidate[0][0]) * (1.0 - neck_ratio)
    y_offset_neck = (candidate[1][1] - candidate[0][1]) * (1.0 - neck_ratio)
    ...
    ########shoulder2########
    l_shoulder2_ref = (
        (ref_candidate[2][0] - ref_candidate[1][0]) ** 2
        + (ref_candidate[2][1] - ref_candidate[1][1]) ** 2
    ) ** 0.5
    l_shoulder2_0 = (
        (candidate[2][0] - candidate[1][0]) ** 2
        + (candidate[2][1] - candidate[1][1]) ** 2
    ) ** 0.5

    shoulder2_ratio = l_shoulder2_ref / l_shoulder2_0

    x_offset_shoulder2 = (candidate[1][0] - candidate[2][0]) * (1.0 - shoulder2_ratio)
    y_offset_shoulder2 = (candidate[1][1] - candidate[2][1]) * (1.0 - shoulder2_ratio)
    ...
    ########shoulder5########
    l_shoulder5_ref = (
        (ref_candidate[5][0] - ref_candidate[1][0]) ** 2
        + (ref_candidate[5][1] - ref_candidate[1][1]) ** 2
    ) ** 0.5
    l_shoulder5_0 = (
        (candidate[5][0] - candidate[1][0]) ** 2
        + (candidate[5][1] - candidate[1][1]) ** 2
    ) ** 0.5

    shoulder5_ratio = l_shoulder5_ref / l_shoulder5_0

    x_offset_shoulder5 = (candidate[1][0] - candidate[5][0]) * (1.0 - shoulder5_ratio)
    y_offset_shoulder5 = (candidate[1][1] - candidate[5][1]) * (1.0 - shoulder5_ratio)
    ...
    ########arm3########
    l_arm3_ref = (
        (ref_candidate[3][0] - ref_candidate[2][0]) ** 2
        + (ref_candidate[3][1] - ref_candidate[2][1]) ** 2
    ) ** 0.5
    l_arm3_0 = (
        (candidate[3][0] - candidate[2][0]) ** 2
        + (candidate[3][1] - candidate[2][1]) ** 2
    ) ** 0.5

    arm3_ratio = l_arm3_ref / l_arm3_0

    x_offset_arm3 = (candidate[2][0] - candidate[3][0]) * (1.0 - arm3_ratio)
    y_offset_arm3 = (candidate[2][1] - candidate[3][1]) * (1.0 - arm3_ratio)
    ...
    ########arm4########
    l_arm4_ref = (
        (ref_candidate[4][0] - ref_candidate[3][0]) ** 2
        + (ref_candidate[4][1] - ref_candidate[3][1]) ** 2
    ) ** 0.5
    l_arm4_0 = (
        (candidate[4][0] - candidate[3][0]) ** 2
        + (candidate[4][1] - candidate[3][1]) ** 2
    ) ** 0.5

    arm4_ratio = l_arm4_ref / l_arm4_0

    x_offset_arm4 = (candidate[3][0] - candidate[4][0]) * (1.0 - arm4_ratio)
    y_offset_arm4 = (candidate[3][1] - candidate[4][1]) * (1.0 - arm4_ratio)
    ...
    ########arm6########
    l_arm6_ref = (
        (ref_candidate[6][0] - ref_candidate[5][0]) ** 2
        + (ref_candidate[6][1] - ref_candidate[5][1]) ** 2
    ) ** 0.5
    l_arm6_0 = (
        (candidate[6][0] - candidate[5][0]) ** 2
        + (candidate[6][1] - candidate[5][1]) ** 2
    ) ** 0.5

    arm6_ratio = l_arm6_ref / l_arm6_0

    x_offset_arm6 = (candidate[5][0] - candidate[6][0]) * (1.0 - arm6_ratio)
    y_offset_arm6 = (candidate[5][1] - candidate[6][1]) * (1.0 - arm6_ratio)
    ...
    ########arm7########
    l_arm7_ref = (
        (ref_candidate[7][0] - ref_candidate[6][0]) ** 2
        + (ref_candidate[7][1] - ref_candidate[6][1]) ** 2
    ) ** 0.5
    l_arm7_0 = (
        (candidate[7][0] - candidate[6][0]) ** 2
        + (candidate[7][1] - candidate[6][1]) ** 2
    ) ** 0.5

    arm7_ratio = l_arm7_ref / l_arm7_0

    x_offset_arm7 = (candidate[6][0] - candidate[7][0]) * (1.0 - arm7_ratio)
    y_offset_arm7 = (candidate[6][1] - candidate[7][1]) * (1.0 - arm7_ratio)
    ...
    ########head14########
    l_head14_ref = (
        (ref_candidate[14][0] - ref_candidate[0][0]) ** 2
        + (ref_candidate[14][1] - ref_candidate[0][1]) ** 2
    ) ** 0.5
    l_head14_0 = (
        (candidate[14][0] - candidate[0][0]) ** 2
        + (candidate[14][1] - candidate[0][1]) ** 2
    ) ** 0.5

    head14_ratio = l_head14_ref / l_head14_0

    x_offset_head14 = (candidate[0][0] - candidate[14][0]) * (1.0 - head14_ratio)
    y_offset_head14 = (candidate[0][1] - candidate[14][1]) * (1.0 - head14_ratio)
    ...
    ########head15########
    l_head15_ref = (
        (ref_candidate[15][0] - ref_candidate[0][0]) ** 2
        + (ref_candidate[15][1] - ref_candidate[0][1]) ** 2
    ) ** 0.5
    l_head15_0 = (
        (candidate[15][0] - candidate[0][0]) ** 2
        + (candidate[15][1] - candidate[0][1]) ** 2
    ) ** 0.5

    head15_ratio = l_head15_ref / l_head15_0

    x_offset_head15 = (candidate[0][0] - candidate[15][0]) * (1.0 - head15_ratio)
    y_offset_head15 = (candidate[0][1] - candidate[15][1]) * (1.0 - head15_ratio)
    ...
    ########head16########
    l_head16_ref = (
        (ref_candidate[16][0] - ref_candidate[14][0]) ** 2
        + (ref_candidate[16][1] - ref_candidate[14][1]) ** 2
    ) ** 0.5
    l_head16_0 = (
        (candidate[16][0] - candidate[14][0]) ** 2
        + (candidate[16][1] - candidate[14][1]) ** 2
    ) ** 0.5

    head16_ratio = l_head16_ref / l_head16_0

    x_offset_head16 = (candidate[14][0] - candidate[16][0]) * (1.0 - head16_ratio)
    y_offset_head16 = (candidate[14][1] - candidate[16][1]) * (1.0 - head16_ratio)
    ...
    ########head17########
    l_head17_ref = (
        (ref_candidate[17][0] - ref_candidate[15][0]) ** 2
        + (ref_candidate[17][1] - ref_candidate[15][1]) ** 2
    ) ** 0.5
    l_head17_0 = (
        (candidate[17][0] - candidate[15][0]) ** 2
        + (candidate[17][1] - candidate[15][1]) ** 2
    ) ** 0.5

    head17_ratio = l_head17_ref / l_head17_0

    x_offset_head17 = (candidate[15][0] - candidate[17][0]) * (1.0 - head17_ratio)
    y_offset_head17 = (candidate[15][1] - candidate[17][1]) * (1.0 - head17_ratio)
    ...
    ########left leg########
    l_ll1_ref = (
        (ref_candidate[8][0] - ref_candidate[9][0]) ** 2
        + (ref_candidate[8][1] - ref_candidate[9][1]) ** 2
    ) ** 0.5
    l_ll1_0 = (
        (candidate[8][0] - candidate[9][0]) ** 2
        + (candidate[8][1] - candidate[9][1]) ** 2
    ) ** 0.5
    ll1_ratio = l_ll1_ref / l_ll1_0

    x_offset_ll1 = (candidate[9][0] - candidate[8][0]) * (ll1_ratio - 1.0)
    y_offset_ll1 = (candidate[9][1] - candidate[8][1]) * (ll1_ratio - 1.0)
    ...
    l_ll2_ref = (
        (ref_candidate[9][0] - ref_candidate[10][0]) ** 2
        + (ref_candidate[9][1] - ref_candidate[10][1]) ** 2
    ) ** 0.5
    l_ll2_0 = (
        (candidate[9][0] - candidate[10][0]) ** 2
        + (candidate[9][1] - candidate[10][1]) ** 2
    ) ** 0.5
    ll2_ratio = l_ll2_ref / l_ll2_0

    x_offset_ll2 = (candidate[10][0] - candidate[9][0]) * (ll2_ratio - 1.0)
    y_offset_ll2 = (candidate[10][1] - candidate[9][1]) * (ll2_ratio - 1.0)
    ...
    ########right leg########
    l_rl1_ref = (
        (ref_candidate[11][0] - ref_candidate[12][0]) ** 2
        + (ref_candidate[11][1] - ref_candidate[12][1]) ** 2
    ) ** 0.5
    l_rl1_0 = (
        (candidate[11][0] - candidate[12][0]) ** 2
        + (candidate[11][1] - candidate[12][1]) ** 2
    ) ** 0.5
    rl1_ratio = l_rl1_ref / l_rl1_0

    x_offset_rl1 = (candidate[12][0] - candidate[11][0]) * (rl1_ratio - 1.0)
    y_offset_rl1 = (candidate[12][1] - candidate[11][1]) * (rl1_ratio - 1.0)
    ...
    l_rl2_ref = (
        (ref_candidate[12][0] - ref_candidate[13][0]) ** 2
        + (ref_candidate[12][1] - ref_candidate[13][1]) ** 2
    ) ** 0.5
    l_rl2_0 = (
        (candidate[12][0] - candidate[13][0]) ** 2
        + (candidate[12][1] - candidate[13][1]) ** 2
    ) ** 0.5
    rl2_ratio = l_rl2_ref / l_rl2_0

    x_offset_rl2 = (candidate[13][0] - candidate[12][0]) * (rl2_ratio - 1.0)
    y_offset_rl2 = (candidate[13][1] - candidate[12][1]) * (rl2_ratio - 1.0)
    ...
    for i in range(1, len(results_vis)):
        ...
        ########neck########
        x_offset_neck = (
            results_vis[i]["bodies"]["candidate"][1][0]
            - results_vis[i]["bodies"]["candidate"][0][0]
        ) * (1.0 - neck_ratio)
        y_offset_neck = (
            results_vis[i]["bodies"]["candidate"][1][1]
            - results_vis[i]["bodies"]["candidate"][0][1]
        ) * (1.0 - neck_ratio)
        ...
        x_offset_shoulder2 = (
            results_vis[i]["bodies"]["candidate"][1][0]
            - results_vis[i]["bodies"]["candidate"][2][0]
        ) * (1.0 - shoulder2_ratio)
        y_offset_shoulder2 = (
            results_vis[i]["bodies"]["candidate"][1][1]
            - results_vis[i]["bodies"]["candidate"][2][1]
        ) * (1.0 - shoulder2_ratio)
        ...
        x_offset_shoulder5 = (
            results_vis[i]["bodies"]["candidate"][1][0]
            - results_vis[i]["bodies"]["candidate"][5][0]
        ) * (1.0 - shoulder5_ratio)
        y_offset_shoulder5 = (
            results_vis[i]["bodies"]["candidate"][1][1]
            - results_vis[i]["bodies"]["candidate"][5][1]
        ) * (1.0 - shoulder5_ratio)
        ...
        x_offset_arm3 = (
            results_vis[i]["bodies"]["candidate"][2][0]
            - results_vis[i]["bodies"]["candidate"][3][0]
        ) * (1.0 - arm3_ratio)
        y_offset_arm3 = (
            results_vis[i]["bodies"]["candidate"][2][1]
            - results_vis[i]["bodies"]["candidate"][3][1]
        ) * (1.0 - arm3_ratio)
        ...
        x_offset_arm4 = (
            results_vis[i]["bodies"]["candidate"][3][0]
            - results_vis[i]["bodies"]["candidate"][4][0]
        ) * (1.0 - arm4_ratio)
        y_offset_arm4 = (
            results_vis[i]["bodies"]["candidate"][3][1]
            - results_vis[i]["bodies"]["candidate"][4][1]
        ) * (1.0 - arm4_ratio)
        ...
        x_offset_arm6 = (
            results_vis[i]["bodies"]["candidate"][5][0]
            - results_vis[i]["bodies"]["candidate"][6][0]
        ) * (1.0 - arm6_ratio)
        y_offset_arm6 = (
            results_vis[i]["bodies"]["candidate"][5][1]
            - results_vis[i]["bodies"]["candidate"][6][1]
        ) * (1.0 - arm6_ratio)
        ...
        x_offset_arm7 = (
            results_vis[i]["bodies"]["candidate"][6][0]
            - results_vis[i]["bodies"]["candidate"][7][0]
        ) * (1.0 - arm7_ratio)
        y_offset_arm7 = (
            results_vis[i]["bodies"]["candidate"][6][1]
            - results_vis[i]["bodies"]["candidate"][7][1]
        ) * (1.0 - arm7_ratio)
        ...
        x_offset_head14 = (
            results_vis[i]["bodies"]["candidate"][0][0]
            - results_vis[i]["bodies"]["candidate"][14][0]
        ) * (1.0 - head14_ratio)
        y_offset_head14 = (
            results_vis[i]["bodies"]["candidate"][0][1]
            - results_vis[i]["bodies"]["candidate"][14][1]
        ) * (1.0 - head14_ratio)
        ...
        x_offset_head15 = (
            results_vis[i]["bodies"]["candidate"][0][0]
            - results_vis[i]["bodies"]["candidate"][15][0]
        ) * (1.0 - head15_ratio)
        y_offset_head15 = (
            results_vis[i]["bodies"]["candidate"][0][1]
            - results_vis[i]["bodies"]["candidate"][15][1]
        ) * (1.0 - head15_ratio)
        ...
        x_offset_head16 = (
            results_vis[i]["bodies"]["candidate"][14][0]
            - results_vis[i]["bodies"]["candidate"][16][0]
        ) * (1.0 - head16_ratio)
        y_offset_head16 = (
            results_vis[i]["bodies"]["candidate"][14][1]
            - results_vis[i]["bodies"]["candidate"][16][1]
        ) * (1.0 - head16_ratio)
        ...
        x_offset_head17 = (
            results_vis[i]["bodies"]["candidate"][15][0]
            - results_vis[i]["bodies"]["candidate"][17][0]
        ) * (1.0 - head17_ratio)
        y_offset_head17 = (
            results_vis[i]["bodies"]["candidate"][15][1]
            - results_vis[i]["bodies"]["candidate"][17][1]
        ) * (1.0 - head17_ratio)
        ...
        ########left leg########
        x_offset_ll1 = (
            results_vis[i]["bodies"]["candidate"][9][0]
            - results_vis[i]["bodies"]["candidate"][8][0]
        ) * (ll1_ratio - 1.0)
        y_offset_ll1 = (
            results_vis[i]["bodies"]["candidate"][9][1]
            - results_vis[i]["bodies"]["candidate"][8][1]
        ) * (ll1_ratio - 1.0)
        ...
        x_offset_ll2 = (
            results_vis[i]["bodies"]["candidate"][10][0]
            - results_vis[i]["bodies"]["candidate"][9][0]
        ) * (ll2_ratio - 1.0)
        y_offset_ll2 = (
            results_vis[i]["bodies"]["candidate"][10][1]
            - results_vis[i]["bodies"]["candidate"][9][1]
        ) * (ll2_ratio - 1.0)
        ...
        x_offset_rl1 = (
            results_vis[i]["bodies"]["candidate"][12][0]
            - results_vis[i]["bodies"]["candidate"][11][0]
        ) * (rl1_ratio - 1.0)
        y_offset_rl1 = (
            results_vis[i]["bodies"]["candidate"][12][1]
            - results_vis[i]["bodies"]["candidate"][11][1]
        ) * (rl1_ratio - 1.0)
        ...
        x_offset_rl2 = (
            results_vis[i]["bodies"]["candidate"][13][0]
            - results_vis[i]["bodies"]["candidate"][12][0]
        ) * (rl2_ratio - 1.0)
        y_offset_rl2 = (
            results_vis[i]["bodies"]["candidate"][13][1]
            - results_vis[i]["bodies"]["candidate"][12][1]
        ) * (rl2_ratio - 1.0)
    ...
    for i in range(len(results_vis)):
        ...
        cv2.imwrite(img_path, dwpose_woface[:, :, [2, 1, 0]])
...
if __name__ == "__main__":
    ...
    def parse_args():
        ...
        parser.add_argument(
            "--ref_name",
            type=str,
            default="data/images/IMG_20240514_104337.jpg",
        )
        parser.add_argument(
            "--source_video_paths",
            type=str,
            default="data/videos/source_video.mp4",
        )
        parser.add_argument(
            "--saved_pose_dir",
            type=str,
            default="data/saved_pose/IMG_20240514_104337",
        )
